...
class BlackjackGame {
  ...
  async placeBet(userId, amount) {
    ...
    try {
      ...
      if (nextBet > this.maxBet) {
        console.log(`⛔ Apuesta rechazada: ${nextBet} > maxBet: ${this.maxBet}`);
        return false;
      }

      player.bet = nextBet;
      player.chips -= amount;
      console.log(`💰 Apuesta: user=${userId}, bet=${player.bet}, chips=${player.chips}`);
  ...
  canStartRound() {
    console.log(`🎯 canStartRound llamado para ${this.id}`);

    this.playerOrder.forEach((id) => {
      const player = this.players[id];
      if (player?.isAI && player.bet === 0) {
        console.log(`🤖 IA ${id} necesita apuesta, bet_actual=${player.bet}`);
        const betAmount = this.calculateKellyBet(id);
        console.log(`🤖 calculateKellyBet retornó: ${betAmount}`);
        if (betAmount > 0) {
          this.placeBet(id, betAmount);
        }
      }
