...
- **Nginx**
  ...
  - Forces **HTTP → HTTPS** redirection. :80 → :443
  - ⚠️ Uses a self-signed SSL certificate for local development: Your browser will show a security warning. Accept the exception to continue.
  - Routes:
    - `/` → frontend
...
- `make` / `make all` → full setup and start (only one command)
